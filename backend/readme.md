...
Create a `.env` file in the root `backend/` directory with the following variable (required for Layer 3's AI Threat Reports):
```bash
...
1. **Test the Central Brain**
   ...
   - Click **Try it out**. Enter a JSON payload that simulates a hacked account (authenticated sender, but highly suspicious content and a brand new domain):
   {
     ...
     "raw_text": "Dear Customer, your DEMAT account KYC has expired. Click here immediately to update your PAN and Aadhar."
   }
   ```
   - Click **Execute**. The Random Forest should flag this as a severe threat, despite the `is_authenticated_sender` being 1, because the text is malicious and the domain is brand new! The response will now also include a `scan_id` (saved to the DB) and a beautifully formatted `llm_threat_report` generated by Groq.
