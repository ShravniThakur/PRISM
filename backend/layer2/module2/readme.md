...
## 🚀 The Pipeline
1. **Media Ingest**: Uses `ffmpeg` to rip `.mp4` / `.mov` files into isolated 16kHz audio streams and video frames.
2. **Vision Processing**: Uses **MTCNN** (Multi-task Cascaded Convolutional Networks) to track and crop faces in the video frames. It also uses **Tesseract OCR** to extract any visible on-screen text.
3. **Deepfake Scoring**: 
   - **Video**: Passes cropped faces through **SigLIP** (`prithivMLmods/deepfake-detector-model-v1`).
...
*   **`media_ingest.py`**: The entry point. It takes raw uploaded video files and uses `ffmpeg` to split them into a temporary `.mp4` video stream and a `16kHz .wav` audio stream.
*   **`vision_processing.py`**: Uses **MTCNN** to scan every frame of the video, detect human faces, crop them out, and save them as PyTorch tensors. Also runs **Tesseract OCR** to grab text.
...
*   **`deepfake_scoring.py`**: The heavy lifter. Loads the 1.5GB of offline HuggingFace models (`SigLIP` and `Wav2Vec2`) into your Mac's MPS GPU memory, runs the tensors through the neural networks, and returns the mathematically accurate Fake/Real probabilities.
...
Before running the test script, ensure your Mac is configured correctly.

### 1. Install System Dependencies (macOS)
You need FFmpeg to split video and Tesseract for OCR.
```bash
brew install ffmpeg
...
We use a standard Python virtual environment to manage dependencies. Ensure you are using Python 3.10 or 3.11.
```bash
# From the root PRISM directory
python3 -m venv .venv
...
### 3. Setup Offline Models
Create a `DeepFakeModels` directory at the root of your project (`PRISM/DeepFakeModels`). It must contain the following file structure for offline inference:

```text
DeepFakeModels/
├── video_deepfake_siglip.safetensors
...
## Sources 
- https://huggingface.co/prithivMLmods/deepfake-detector-model-v1/tree/main
